...
#define UDP_LAT_RAW_SAMPLES    1024
#define UDP_LAT_SAMPLE_STRIDE  128
...
struct udp_lat_stats {
	...
	u64 raw_samples[UDP_LAT_RAW_SAMPLES];
	u32 raw_head;
	u32 sample_counter;
};

static DEFINE_PER_CPU(struct udp_lat_stats, udp_sendmsg_lat_stats);
...
static inline void udp_sendmsg_lat_record(u64 delta_ns)
{
	struct udp_lat_stats *s;
	u32 bucket;

	s = this_cpu_ptr(&udp_sendmsg_lat_stats);
	...
	if ((s->sample_counter++ % UDP_LAT_SAMPLE_STRIDE) == 0) {
		s->raw_samples[s->raw_head] = delta_ns;
		s->raw_head = (s->raw_head + 1) % UDP_LAT_RAW_SAMPLES;
	}
}
...
static void udp_sendmsg_lat_aggregate(struct udp_lat_agg *agg)
{
	int cpu;
	bool first = true;
	...
	for_each_possible_cpu(cpu) {
		struct udp_lat_stats *s = per_cpu_ptr(&udp_sendmsg_lat_stats, cpu);
		int i;
		...
		if (first) {
			...
			if (s->min_ns < agg->min_ns)
				agg->min_ns = s->min_ns;
			if (s->max_ns > agg->max_ns)
				agg->max_ns = s->max_ns;
		}
...
{
	struct udp_lat_agg agg;
	u64 avg_ns = 0;
	u64 p50, p95, p99;
	int cpu, i;

	udp_sendmsg_lat_aggregate(&agg);

	if (agg.count)
		avg_ns = div64_u64(agg.total_ns, agg.count);

	p50 = udp_sendmsg_lat_percentile_ns(&agg, 50);
	p95 = udp_sendmsg_lat_percentile_ns(&agg, 95);
	p99 = udp_sendmsg_lat_percentile_ns(&agg, 99);

	seq_puts(m, "udp_sendmsg latency stats\n");
	...
	for_each_possible_cpu(cpu) {
		...
		n = min_t(u32, s->raw_head, UDP_LAT_RAW_SAMPLES);
		start = (s->raw_head >= n) ? (s->raw_head - n) : 0;

		for (; start < s->raw_head; start++)
			seq_printf(m, " %llu", s->raw_samples[start]);
...
out_free:
	...
	udp_sendmsg_lat_record(__lat_t1 - __lat_t0);
	// End test
