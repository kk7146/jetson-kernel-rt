...
static int __udp_cmsg_send(struct cmsghdr *cmsg, u16 *gso_size) // 내부 헬퍼
{
	switch (cmsg->cmsg_type) { // 타입이 여러 개가 들어올거라 생각했나?
	case UDP_SEGMENT:
...
int udp_cmsg_send(struct sock *sk, struct msghdr *msg, u16 *gso_size)
{
	...
	for_each_cmsghdr(cmsg, msg) { // include/linux/socket.h에 정의되어 있음.
		if (!CMSG_OK(msg, cmsg))
			return -EINVAL;
...
int udp_sendmsg(struct sock *sk, struct msghdr *msg, size_t len)
{
	...
	//	}
	// 이것도 캐스팅인 듯?
	// include/net/udp.h:188
	struct udp_sock *up = udp_sk(sk);

	// #define DECLARE_SOCKADDR(type, dst, src)	type dst = ({ __sockaddr_check_size(sizeof(*dst)); (type) src; })
	// type: sockaddr_in 포인터 타입, dst: usin, src: msg->msg_name
	// usin에 sockaddr_in 포인터 타입으로 하나 생성하고 src를 그 타입으로 캐스팅해서 대입.
	// include/linux/net.h:205
	...
	int free = 0;
	int connected = 0; // 연결된 udp 소켓이라는 거..
	__be32 daddr, faddr, saddr;
	__be16 dport;
	u8  tos;
	// UDP Lite인지 아닌지 확인하는 그런 함수.
	// #define IS_UDPLITE(__sk) (__sk->sk_protocol == IPPROTO_UDPLITE)로 정의. (include/linux/udp.h:189)
	// IPPROTO_UDPLITE은 UDP-Lite의 프로토콜 번호 상수.
	...
	// #endif
	//include/asm-generic/rwonce.h:47
	// 결과적으로 이 시점에 확실히 램에서 가져와서 읽으라는 뜻이 맞을 듯..? 재사용하지 말라는 뜻도 되는 거 같고.
	...
	// MSG_OOB 플래그가 켜져있으면 -EOPNOTSUPP를 반환하면서, 호환되지 않는 것을 알림. Out-Of-Band로, 긴급할 때 보내는 메세지인데..
	// UDP 쪽에서는 쓰이지 않는 것으로 앎.
	if (msg->msg_flags & MSG_OOB) /* Mirror BSD error message compatibility */
		return -EOPNOTSUPP;

	// UDP와 UDP-Lite: 패킷의 일부만 체크섬으로 보호할 수 있다
	...
	if (usin) { // usin이 존재하면 여기서 주소 꺼냄 .
		...
		if (usin->sin_family != AF_INET) { // IPv4?
			if (usin->sin_family != AF_UNSPEC) // Unspecified. IPv4라고 명시하지는 않았지만, 특별히 다른 주소 계열도 아니니 일단 받는다 라는 의미인 듯.
				return -EAFNOSUPPORT;
		}
	...
	if (msg->msg_controllen) { // UDP/IP control message(cmsg) 가 있으면 그걸 먼저 반영
		...
		if (unlikely(err < 0)) { // 에러 체크인데 그럴 확률이 높지 않다는 힌트를 줌.
			kfree(ipc.opt);
			return err;
		}
		if (ipc.opt)
	...
	if (!ipc.opt) { // 그래도 ipc.opt가 없으면 소켓 기본 IP 옵션을 가져오는 거
		struct ip_options_rcu *inet_opt; // rcu는 Read-Copy-Update의 약자로, 락 없이 데이터를 읽을 수 있게 해주는 기술인데?
		//Read: 읽는 쪽은 잠깐 보호 구간 안에서 포인터를 읽음
		// Copy: 쓰는 쪽은 기존 객체를 직접 덮어쓰지 않고 새 복사본을 만듦
		// Update: 포인터를 새 객체로 바꿈
		// 그다음 한동안 기다렸다가, 예전 객체를 아무도 안 읽는 시점에 해제함
		// 저게 공유 포인터라 rcu로 읽는 것으로 합의 본 듯 합니다.
	...
	if (cgroup_bpf_enabled(CGROUP_UDP4_SENDMSG) && !connected) { // cgroup_bpf_enabled는 그냥 켜져있는지 확인하는 함수.
		...
		if (err) // BPF 프로그램이 sendmsg를 막아버리는 경우
			goto out_free;
		if (usin) { // BPF 프로그램이 목적지 주소를 바꿔버리는 경우
			if (usin->sin_port == 0) {
	...
	if (ipc.opt && ipc.opt->opt.srr) { // 중간에 경유지를 두는 옵션 daddr이 목적지이고 중간에 faddr을 추가함.
		...
		connected = 0; // 중간에 경유지를 둘 때 fast path로 같은 경로 쓰는 걸 막음.
	}
	tos = get_rttos(&ipc, inet); // route lookup에서 참고할 라우팅 속성 값
	...
	if (ipv4_is_multicast(daddr)) { // 멀티캐스트일 때
		...
		if (!saddr)
			saddr = inet->mc_addr;
		connected = 0;
	} else if (!ipc.oif) { // 일반 유니캐스트일 때
		ipc.oif = inet->uc_index;
	} else if (ipv4_is_lbcast(daddr) && inet->uc_index) { // 로컬 브로드캐스트일 때
